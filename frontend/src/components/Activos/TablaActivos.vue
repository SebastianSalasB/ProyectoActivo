<template>
  ...
  <b-container fluid>
            ...
            > 
              ...
              <template #cell(acciones)="data">
                ...
                >
                  ...
                  <b-dropdown-item 
                    v-if="['mantenimiento'].includes(data.item.act_estado)"
                    @click="abrirModalActivacion(data.item)" 
                  >
    ...
    <!-- Modal de activacion -->
    <b-modal v-model="mostrarModal" title="Registrar fecha de activación" @ok="confirmarActivacion" ok-title="activar" cancel-title="Cancelar" ok-variant="primary" centered>
      <b-form >
        <b-form-group label="Fecha de activación">
          <b-form-input type="date" v-model="fechaActivacion" required />
        </b-form-group>
...
export default {
  ...
  computed: {
    ...
    tablaMantencion() {
      return [
        { key: 'man_id', label: 'ID' },
        { key: 'man_fecha', label: 'Fecha mantencion' },
        { key: 'man_fecha_salida', label: 'Fecha activacion' },
